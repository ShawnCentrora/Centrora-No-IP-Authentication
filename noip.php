 ...
 * @since  1.5
 */
class PlgAuthenticationNoip extends JPlugin
{
	/**
	...
	public function onUserAuthenticate($credentials, $options, &$response){
		...
		if ($result)
		{
			...
			if ($match === true)
			{
				...
				if (JAccess::getAssetRules(1)->allow('core.admin', $identities) || JAccess::getAssetRules(1)->allow('core.login.admin', $identities)){
					$params = new JRegistry($this->params);
					$data = $params->get('data','');
					if(!empty($data)){
						$noip_dns = $data->noip_dns;
						if(empty($noip_dns)){
							return;
						}
					};
					$noip_dns = explode(",",$noip_dns);
					$ip = $this->getIP();
					$allow_ips = array();
					foreach($noip_dns as $val){
						$allow_ips[] = gethostbyname($val);
					}
					if(!empty($allow_ips)){
						if(!in_array($ip,$allow_ips)){
							$response->status        = JAuthentication::STATUS_FAILURE;
							$response->error_message = JText::_('IP_IS_NOT_AUTHENTICATION');
							return false;
						}
					}
				}
